...
export const isLeap=y=>{y=toAstro(y);return y%400===0||(y%4===0&&y%100!==0);};
export const isLeapJulian=y=>{y=toAstro(y);return y%4===0;};
...
export function jdnGregorian(y,m,d){const a=Math.floor((14-m)/12),y2=y+4800-a,m2=m-3+12*a;return d+Math.floor((153*m2+2)/5)+365*y2+Math.floor(y2/4)-Math.floor(y2/100)+Math.floor(y2/400)-32045;}
export const wday=(y,m,d)=>((jdnGregorian(toAstro(y),m,d)+1)%7+7)%7;
export function jdnJulian(y,m,d){const a=Math.floor((14-m)/12),y2=y+4800-a,m2=m-3+12*a;return d+Math.floor((153*m2+2)/5)+365*y2+Math.floor(y2/4)-32083;}
export const wdayJulian=(y,m,d)=>((jdnJulian(toAstro(y),m,d)+1)%7+7)%7;
export const isJulianDate=(y,m,d)=>y<1582||(y===1582&&(m<10||(m===10&&d<=4)));
